...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def createConfigContent(self):
        ...
        configContent = ['默认，可更改为自己需要的', 'Z:\\Inorganic\\Program\\1.Inorganic Operate\\1.New edition\\2.Model',
                         'REACH_SVHC_Candidate_List_Opinion.csv']
                ...
                for n in range(len(reachLimsNo)):
                    ...
                    if m == 'T':
                        if self.checkBox.checkState():
                            self.textBrowser.append(
                                "<table border='1'> <tr><th>目标</th> <th>内容</th></tr> <tr><td>Reach Lims No:</td><td>%s</td></tr> <tr><td>Reach 中文名:</td><td>%s</td></tr> <tr><td>Reach 英文名:</td><td>%s</td></tr> <tr><td>Reach CAS No:</td><td><font color='red'>%s</font></td></tr> <tr><td>Reach 物质作用:</td><td>%s</td></tr> </table>" % (
                                reachLimsNo[num], reachChinese[num], reachEnglish[num], reachCas[num],
                                reachPurpose[num]))
                            self.textBrowser.append('--------------------------')
                            app.processEvents()
...
class MyTableWindow(QMainWindow, Ui_TableWindow):
    ...
    def searchReachMessage(self):
        ...
        maybeC = ['Natural textiles', 'Synthetic textiles', 'Leather', 'Metal', 'Plastic|polymers|foam', 'Wood',
                  'Paper', 'Ceramic', 'Glass', 'Dye|Pigment|Ink|Paint', 'Adhesives|Sealants', 'Battery',
        ...
        if material != '':
            if estimate != '':
                if estimate == 'Y':
                    searchReachMessage = searchReachMessage.loc[
                        (searchReachMessage[material] == 'Y') | (searchReachMessage[material] == 'Y*')]
                else:
                    searchReachMessage = searchReachMessage.loc[
                        (searchReachMessage[material] == 'N') | (searchReachMessage[material] == 'N*')]
